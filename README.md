...
Once the middleware stage is completed, the request is passed to the route handlers associated with the requested path.
The response generated by the final handler is then sent back to the client through the corresponding HTTP/3 stream.
...
Instead, application-specific logic is injected through a user-defined `AppState` (identified as `S` generic, which implements `Send` + `Sync`, and guarantees a `'static` lifetime) type when creating the router. This allows developers to integrate their own storage systems or application logic without modifying the server infrastructure.
